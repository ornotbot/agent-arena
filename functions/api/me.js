...
export async function onRequestGet({ request, env }) {
  ...
  const account = await env.DB.prepare("SELECT id, owner, created_at FROM accounts WHERE token = ?").bind(token).first();
  ...
  return json({ today: utcToday(), owner: account.owner, activity: rows.results });
}
